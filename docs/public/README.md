# Public FireViewer documentation

This is the tracked, canonical project-wide material intended for users,
partners, reviewers and contributors evaluating FireViewer.

The documents keep their stable paths one level above this index so existing
public links do not break. It deliberately excludes working notes, raw logs,
machine-specific instructions, private incident material and unqualified
experiment reports; those remain local and are never part of this index.

## Project and safety

- [Project overview](../PROJECT_OVERVIEW.md)
- [Architecture](../ARCHITECTURE.md)
- [Status matrix](../STATUS_MATRIX.md)
- [Roadmap](../ROADMAP.md)
- [Safety and scope](../SAFETY_AND_SCOPE.md)
- [Terminology](../TERMINOLOGY.md)

## Spatial, temporal and reproducibility

- [Map Builder](../MAP_BUILDER.md)
- [Fire evolution timeline](../FIRE_EVOLUTION_TIMELINE.md)
- [Replay and post-event studies](../REPLAY_AND_POST_EVENT_STUDIES.md)
- [Provenance and reproducibility](../PROVENANCE_AND_REPRODUCIBILITY.md)
- [Models, datasets and benchmarks](../MODELS_DATASETS_BENCHMARKS.md)

## Contracts, security and publication

- [Contract map](../CONTRACT_MAP.md)
- [Acceptance and replay matrix](../ACCEPTANCE_AND_REPLAY_MATRIX.md)
- [External source connectors](../EXTERNAL_SOURCE_CONNECTORS.md)
- [Security and privacy](../SECURITY_PRIVACY_EVENT_PIPELINE.md)
- [Storage and retention](../STORAGE_AND_RETENTION.md)
- [Event API v2](../contracts/EVENT_API_V2.md)
- [Event and provenance v2](../contracts/EVENT_AND_PROVENANCE_V2.md)

## Support and licensing

- [Funding brief](../FUNDING_BRIEF.md)
- [Brief financement](../FUNDING_BRIEF_FR.md)
- [Funding application kit](../FUNDING_APPLICATION_KIT.md)
- [Support and partnerships](../SUPPORT_AND_PARTNERSHIPS.md)
- [Licensing and citation](../LICENSING.md)
