...
| Source repository hygiene | Public Git trees are limited to source, documentation, portable configuration/contracts and bounded synthetic test fixtures. The Unreal tree keeps one invented JSON/GeoJSON incident example. | Datasets, weights, private data, imported assets, generated maps and reproductions remain outside Git. Cleanup preserves referenced compatibility paths until an explicit migration exists. |
