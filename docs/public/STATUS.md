...
| Source repository hygiene | Git trees are limited to source, documentation, portable configuration/contracts and bounded test fixtures. The private frontend/backend retain one reviewed Die / Justin demonstration; the Unreal tree keeps one invented JSON/GeoJSON incident example. | Datasets, weights, private data, source media, imported assets, generated maps and reproductions remain outside Git. Cleanup preserves referenced compatibility paths until an explicit migration exists. |
