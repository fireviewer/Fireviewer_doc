...
**Public status snapshot: 4 September 2026.**

**Incident web atlas update: 6 September 2026.** The other areas below retain
their 4 September assessment.
...
| Visual detection | A provisional CPU visual-detection stage emits image-space boxes and scores. RT-DETR v2 R50 and D-FINE Large fire/smoke checkpoints are publicly visible research resources on Hugging Face. | Public visibility is not runtime promotion or scientific qualification. Visual detection is not geographic localisation and no detector is publication authority. |
...
| Incident web atlas · 6 September | Eleven published terrain views loaded in Chrome; eight have incident geometry. Die / Justin has 59 documentary daily states, 35 sourced chronology entries and dated ground-photo markers. Compact desktop and mobile controls are deployed. [View every map capture](WEB_MAPS.md). | Saumos, Gros Bessillon and Cagnano have no published incident geometry. A complete daily calendar does not establish a complete measured boundary every day. This curated release does not qualify Part.4 or independently validate reconstructed states. |
| Measured-map production | The provider-neutral Map Builder supports resumable tile shards, deterministic checkpoint ownership, a dependent final assembler, tiled viewer packages and asset-free production. Portable Unreal export contracts, a source-only Unreal consumer and a guarded backend `aws_unreal` worker adapter are present. | Source and unit-test evidence do not qualify a packaged Unreal build, cloud worker, visual result, every map size, browser or publication route. |
...
| Source repository hygiene | Git trees are limited to source, documentation, portable configuration/contracts and bounded fixtures. The incident frontend also retains the authorised static documentary web release; this documentation contains selected, requested public atlas screenshots. The Unreal tree keeps one invented JSON/GeoJSON incident example. | The atlas exception does not admit raw source archives, private evidence, weights, measured terrain packages or reproduction corpora. Cleanup preserves referenced compatibility paths until an explicit migration exists. |
